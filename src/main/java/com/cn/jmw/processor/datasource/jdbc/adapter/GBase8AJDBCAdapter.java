 ...
 */
@Slf4j
public class Gbase8JdbcAdapter extends AbstractJdbcAdapter {
     ...
     * @param test           是否为测试模式
     */
    public Gbase8JdbcAdapter(String hostname, Integer port, String databaseName, String username, String password, JdbcAdapterDataSourceConfig config, String connectionUser, Boolean test, ConnectionStringStrategy strategy) {
        super(hostname, port, databaseName, username, password,config,connectionUser,test,strategy);
    }

    public Gbase8JdbcAdapter(String hostname, Integer port, String databaseName, String username, String password, JdbcAdapterDataSourceConfig config, Boolean test, ConnectionStringStrategy strategy) {
...
//     * 构造函数，使用默认连接用户。
//     */
//    public Gbase8JdbcAdapter(String hostname, Integer port, String databaseName, String username, String password, JdbcAdapterDataSourceConfig config, Boolean test) {
...
//     * 构造函数，使用默认配置。
//     */
//    public Gbase8JdbcAdapter(String hostname, Integer port, String databaseName, String username, String password, Boolean test)  {
...
//    }
    ...
    public String generateConnectionString() {
        return String.format("jdbc:gbase://%s:%d/%s?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&autoReconnect=true&nullCatalogMeansCurrent=true",
    ...
    public DatabaseEnum getDatabaseType() {
        return DatabaseEnum.GBASE8A;
    }

    /**
     * 添加伪随机抽样逻辑。
     * <p>
     * GBase 8a 支持 LIMIT 实现伪随机抽样。本方法通过计算总数并结合 LIMIT 返回前 m 行数据，不保证随机性。
     * </p>
     *
